# Author: Dezso Modos, Lejla Gul
# Date: April 2020 (modified on June 2022)
#
...
path <- file.path(outdir, "1_process_a_priori_networks", "unprocessed_networks")
...
# If using only the core OmniPath
interactions <- ia_omnipath

# Otherwise, if you need higher coverage on PPI add these with `bind_rows` below
...
human_directed_interactome <- igraph::simplify(human_directed_interactome, remove.multiple=FALSE)
...
human_directed_interactome_filtered = igraph::as_data_frame(human_directed_interactome_giant_component, what = "edges")
...
ia_transcriptional2 <- igraph::simplify(ia_transcriptional2, remove.multiple = FALSE, remove.loops = TRUE)
...
human_tf_data_filtered = igraph::as_data_frame(ia_transcriptional2, what = "edges")
